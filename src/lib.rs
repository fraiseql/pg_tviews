...
static JSONB_DELTA_AVAILABLE: AtomicBool = AtomicBool::new(false);
static JSONB_DELTA_CHECKED: AtomicBool = AtomicBool::new(false);
...
pub fn check_jsonb_delta_available() -> bool {
    ...
    if JSONB_DELTA_CHECKED.load(Ordering::Relaxed) {
        return JSONB_DELTA_AVAILABLE.load(Ordering::Relaxed);
    }
    ...
    // Cache result
    JSONB_DELTA_AVAILABLE.store(is_available, Ordering::Relaxed);
    JSONB_DELTA_CHECKED.store(true, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_pg_tviews_works_without_jsonb_delta() {
        ...
        // Test that our check function works
        let result = Spi::get_one::<bool>("SELECT pg_tviews_check_jsonb_delta()");
        // This should work even without jsonb_delta
        assert!(result.is_ok(), "pg_tviews should work without jsonb_delta");
        assert!(!result.unwrap().unwrap_or(true), "jsonb_delta should not be available");
    }
}
// */

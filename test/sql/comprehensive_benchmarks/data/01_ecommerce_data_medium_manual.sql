-- E-Commerce Data Generation (Manual Setup Version - Medium Scale)
-- Modified to work without pg_tviews extension
-- Generates data for approaches 3 & 4 only: 100 categories, 100K products, 500K reviews
...
BEGIN
    RAISE NOTICE 'Generating MEDIUM scale data: % categories, % suppliers, % products, % reviews',
        v_num_categories, v_num_suppliers, v_num_products, v_num_reviews;
    RAISE NOTICE 'WARNING: This may take 2-5 minutes and requires ~4GB RAM';
    ...
    SELECT
        'Category ' || i,
        'category-' || i,
        ((i - 21) % 20) + (SELECT MIN(pk_category) FROM tb_category WHERE fk_parent_category IS NULL)
    FROM generate_series(21, v_num_categories) AS i;
    ...
    FOR i IN 0..(v_num_products / v_batch_size - 1) LOOP
        ...
        SELECT
            ...
            'SKU-' || LPAD((i * v_batch_size + j)::TEXT, 8, '0'),
            'Product ' || (i * v_batch_size + j),
            'Description for product ' || (i * v_batch_size + j) || '. This is a high-quality product with excellent features.',
    ...
    IF v_current_count < v_num_products THEN
        ...
        SELECT
            (SELECT pk_category FROM tb_category ORDER BY random() LIMIT 1),
            CASE WHEN random() < 0.8 THEN (SELECT pk_supplier FROM tb_supplier ORDER BY random() LIMIT 1) ELSE NULL END,
            'SKU-' || LPAD((v_current_count + j)::TEXT, 8, '0'),
            'Product ' || (v_current_count + j),
            'Description for product ' || (v_current_count + j) || '. This is a high-quality product with excellent features.',
            (random() * 500 + 10)::NUMERIC(10, 2),
            (random() * 500 + 10)::NUMERIC(10, 2),
            CASE WHEN random() < 0.9 THEN 'active' ELSE 'inactive' END
    ...
    FOR i IN 0..(v_num_reviews / v_batch_size - 1) LOOP
        ...
        SELECT
            ...
            'Review content ' || (i * v_batch_size + j) || '. ' || repeat('This product is great. ', 15),
            random() < 0.7,
            (random() * 100)::INTEGER
        FROM generate_series(1, v_batch_size) AS j;
    ...
    IF v_current_count < v_num_reviews THEN
        ...
        SELECT
            ...
            'Review content ' || (v_current_count + j) || '. ' || repeat('This product is great. ', 15),
            random() < 0.7,
            (random() * 100)::INTEGER
    ...
    RAISE NOTICE 'Data generation complete in %.2f seconds (%.2f minutes)',
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Ready to run medium scale manual benchmarks!';
END $$;

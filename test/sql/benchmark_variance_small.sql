-- Variance test: Small cascade (unpopular author with few posts/comments)
-- Tests performance with minimal cascade impact
...
DECLARE
    ...
    duration_ms numeric;
BEGIN
    -- Find an author with minimal activity (1-2 posts, few comments)
    ...
    SET name = 'Small Cascade Author ' || test_author_id,
        email = 'small' || test_author_id || '@example.com'
    ...
    RAISE NOTICE 'SMALL CASCADE BASELINE: %.2f ms (%.2f ms per row)', duration_ms, duration_ms / GREATEST(affected_posts + affected_comments, 1);

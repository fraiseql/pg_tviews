...
2. Patch the corresponding `tv_entity` row via `jsonb_smart_patch_scalar`.
...
3. If row exists:
   ...
   * Patch `tv_entity` row using `jsonb_smart_patch_scalar`.
...
pub fn refresh_entity_pk(key: &crate::RefreshKey) -> spi::Result<()> {
    ...
    Spi::connect(|client| {
        ...
        // 2. Apply patch using jsonb_smart_patch_scalar
        let mut set_fragments: Vec<String> = Vec::new();
        set_fragments.push("data = jsonb_smart_patch_scalar(data, $1)".to_string());
...
* Refresh uses `v_entity` to recompute and `jsonb_smart_patch_scalar` to apply.
...
4. `refresh_entity_pk` (recompute + jsonb_smart_patch_scalar + delete-if-missing)

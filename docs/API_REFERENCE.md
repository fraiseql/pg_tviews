...
**Description**:
Creates a new transactional view (TVIEW) from a SELECT statement. The TVIEW will automatically maintain consistency with its base tables through triggers.
...
-- Returns: 'TVIEW ''user_posts'' created successfully'
```

**Notes**:
- TVIEW name must start with `tv_` (this will be enforced)
...
**Example**:
```sql
SELECT pg_tviews_drop('user_posts');
-- Returns: 'TVIEW ''user_posts'' dropped successfully'

SELECT pg_tviews_drop('nonexistent', true);
-- Returns: 'TVIEW ''nonexistent'' dropped successfully' (no error)
```
...
**Example**:
```sql
SELECT pg_tviews_infer_types('users', ARRAY['id', 'name', 'created_at']);
```
...
SELECT * FROM pg_tviews_infer_types('users', ARRAY['id', 'name']);
```
